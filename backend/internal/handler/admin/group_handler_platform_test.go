...
// 回归分组平台枚举:kimi/zhipu/deepseek/opencode_go 以及本地 happy_shrimp/copilot
// 必须能通过 Create/Update 的 binding 校验（历史 bug:调度/路由链路已支持这些平台,
// 但 oneof 白名单漏加,导致平台分组无法创建、账号"无可用分组"）;非法值仍须被拒。
...
func TestGroupPlatformBinding_AllowedPlatforms(t *testing.T) {
	allowed := []string{
		...
		"happy_shrimp", "copilot", "kimi", "zhipu", "deepseek", "minimax", "opencode_go", "composite",
	}
